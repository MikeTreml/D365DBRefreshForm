...
namespace SAPIENTypes
{
	...
	function Install-D365foDbatools
	{
        ...
        $labelInfo = "Installing PowerShell modules d365fo.tools and dbatools"
	...
	$buttonRun_Click = {
		...
		if ($txtLink.Text -ne '')
		{
			...
			if ($BacpacSasLinkFromLCS.StartsWith('http'))
			{
                ...
                $labelInfo = "Downloading file" + $TempFileName 
        ...
        $labelInfo = "Stopping D365FO environment"
		Stop-D365Environment -All -Kill 
		$mainprogressbaroverlay.PerformStep()
        $labelInfo = "Enable-D365Exception"
		...
		$labelInfo = "Stop-D365Environment"
        Stop-D365Environment -All -Kill 
        $labelInfo = "Switch-D365ActiveDatabase"
		Switch-D365ActiveDatabase -NewDatabaseName $NewDB

		$mainprogressbaroverlay.PerformStep() 
        $labelInfo = "Remove-D365Database"
        ...
        $labelInfo = "Start-D365Environment"
		Start-D365Environment 
		$mainprogressbaroverlay.PerformStep()
		#move the file
        $labelInfo = "Stop-Service MSSQLSERVER, SQLSERVERAGENT"
		...
		if ($oldFile -ne '')
		{
            $labelInfo = "Move-Item -Path $oldFile -Destination G:\MSSQL_DATA\AxDB_Primaryold_" + $dt
			Move-Item -Path $oldFile -Destination G:\MSSQL_DATA\AxDB_Primaryold_$dt.mdf
			#Remove-D365Database -DatabaseName 'AxDB_Original'
		}
        $labelInfo = "Start-Service MSSQLSERVER, SQLSERVERAGENT "
		...
		if ($checkboxCleanUpPowerBISettin.Checked)
		{
			...
			$labelInfo = "Cleaning up Power BI settings"
		...
		{
            $labelInfo = "Enabling SQL Change Tracking"
		...
		{
            $labelInfo = "Setting up new Admin"
		...
		{
            $labelInfo = "Truncate Batch Tables"
		...
		{
            $labelInfo = "Disabling all current Batch Jobs"
		...
		{
            $labelInfo = "Executing Database Sync"
			## Run Database Sync
		...
		{
            $labelInfo = "Setting DB Recovery Model to Simple"
		...
		{
            $labelInfo = "Enable all users except Guest"
		...
		{
            $labelInfo = "List Out User Email Addresses"
	...
	$formDatabaseRefreshFromB.Controls.Add($labelInfo)
	...
	$labelInfo.Location = New-Object System.Drawing.Point(6, 320)
	$labelInfo.Margin = '0, 0, 0, 0'
	$labelInfo.Name = 'labelInfo'
	$labelInfo.Size = New-Object System.Drawing.Size(284, 60)
